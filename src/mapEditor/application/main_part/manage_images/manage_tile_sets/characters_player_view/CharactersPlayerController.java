...
public class CharactersPlayerController implements Controller, CharacterPlayerListener {
  ...
  public CharactersPlayerController(ICharacterPlayerView view, ImageMatrix imageMatrix, Color strokeColor, Color fillColor) {
    this.view = view;
  ...
  @Override
  public void bind() {
    characterCanvas = new CharacterCanvas(imageMatrix.getImage(), view.isHorizontal(), strokeColor, fillColor, this);
  ...
  private void addListeners() {
    view.getPlayButton().setOnAction(event -> {
      if (animation == null) {
        animation = new SpriteAnimation(imageView, new Duration(1000), 3, 4, 32, 32, view.isHorizontal(), characterCanvas.getStartIndex());
